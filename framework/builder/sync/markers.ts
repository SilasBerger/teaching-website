...
function getSortedApplicableMarkers(node: SourceNode, markersDefinition: MarkersDefinition): MarkerWithSpecificity[] {
...
function verifyNoMultipleMarkersWithSameSpecificity(markers: MarkerWithSpecificity[], node: SourceNode) {
  ...
  Optional.of(
    Object.entries(markersBySpecificity).find(([_, matches]) => matches.length > 1)
  ).ifPresent(([specificity, matches]) => {
...
function mapMarkersDefinitionToMarkersWithSpecificity(markersDefinition: MarkersDefinition): MarkerWithSpecificity[] {
  return Object
