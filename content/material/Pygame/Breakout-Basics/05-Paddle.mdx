...
Wir legend dazu als Erstes eine y-Koordinate fest, wo sich das Paddle später befinden soll (Zeile `14`). Dann erstellen wir auch für das Paddle eine Klasse (Zeilen `53`-`59`), sowie eine Instanz und eine Sprite-Gruppe (Zeilen `65`-`66`), analog zum Ball. Ebenfalls analog zum Ball müssen wir noch dafür sorgen, dass das Paddle im Spielfeld angezeigt und updated wird, was wir auf den Zeilen `77`-`78` erledigen.
