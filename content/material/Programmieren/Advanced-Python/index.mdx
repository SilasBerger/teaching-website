...
- Die Klasse `Hund` hat einen _Konstruktor_ (Zeilen `2`-`3`), der die Parameter `self` und `name` entgegennimmt.
  ...
  - Der Wert des Parameters `name` wird der _Instanzvariable_ `self.name` zugewiesen.
- Auf den Zeilen `5`-`6` wird die _Methode_ `mach_wuff()` definiert.
...
- Auf den Zeilen `8`-`9` wird die _Methode_ `sag_wie_du_heisst()` definiert.
  - Auf Zeile `9` wird über den speziellen Parameter `self` auf die _Instanzvariable_ `self.name` zugegriffen.
  - Der Output der Methode `sag_wie_du_heisst()` kann also für jede _Instanz_ der Klasse `Hund` unterschiedlich sein. Er hängt davon ab, welchen Wert wir bei der _Instanziierung_ dem Konstruktor für den Parameter `name` übergeben haben. Diesen Wert nennt man ein _Konstruktor-Argument_.
...
- Auf Zeile `11` wird eine _Instanz_ der `Hund`-Klasse mit dem _Konstruktor-Argument_ `"Benno"` erzeugt.
...
- Auf Zeile `12` wird eine _Instanz_ der `Hund`-Klasse mit dem _Konstruktor-Argument_ `"Manu"` erzeugt.
