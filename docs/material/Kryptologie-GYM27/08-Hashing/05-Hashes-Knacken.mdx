...
Was fällt Ihnen in dieser Tabelle auf? Wie könnten Sie diese Erkenntnis unter Umständen nutzen, um doch auf
das ursprüngliche Passwort zu schliessen?
...
Alle Benutzer in der obigen Benutzerdatenbank haben ein unsicheres, häufig verwendetes Passwort gewählt: jeder
dieser Passwort-Hashes ist in den einschlägigen _Rainbow Tables_ zu finden.
...
Sie kennen bereits die Passwörter von `alice`, `charlie` und `tammy`. Nun wechseln Sie von der bösen auf die
gute Seite.
