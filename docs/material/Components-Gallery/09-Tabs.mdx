...
::::Tabs :::Tab[Windows]{value="win"} :::insight[Paths on Windows :mdi[linux]] On Windows :mdi[microsoft-windows], paths take the form `C:\foo\bar`. :::

:::Tab[macOS]{value="macos"} :::insight[Paths on macOS] On macOS :mdi[apple], paths are specified in the Unix-like style `/foo/bar` :::

:::Tab[Linux]{value="linux"} :::insight[Paths on Linux] On Linux :mdi[linux], just like on macOS, we use Unix-like paths, such as `/foo/bar`. ::: ::::
