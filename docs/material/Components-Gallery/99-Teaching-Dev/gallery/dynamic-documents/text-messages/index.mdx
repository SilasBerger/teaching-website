...
`id` : Die eindeutige ID des dynamischen `DocumentRoot` `name` : Der Namen des Raums. Dieser wird bspw. im Chat angezeigt. `type` : Der Typ des Raums. Dieser bestimmt, welche Komponenten und welche Logik für den Raum verwendet wird.

:::warning[Aktuelle Einschränkungen] Bei der Erstellung eines `dynamic_document_roots` werden die Berechtigungen auf `access=RO_DocumentRoot` und `sharedAccess=RW_DocumentRoot` gesetzt - was vernünftige Standards sind. Allerdings kann dadurch nicht direkt ein Dokument erzeugt und hinzugefügt werden, da auch für ADmins die Berechtigungen nur `RO` sind.
...
export type DocumentTypes =
    /* ... */
    TextMessage | DynamicDocumentRootModel | DynamicDocumentRoots;
```
...
::: :::info[`docusaurus.config.ts`] Die Landing-Page für die Räume `/rooms/<documentRootId>/<dynamicDocumentRootId>` muss im Docusaurus Router registriert werden.
