...
`string` : `<String />` Komponente [String](./string-answers/index.mdx) : Einzeilige Antwort : das String-Attribute `type` wird über `<Answer inputType="..." />` definiert `text` : `<QuillV2 />` Komponente [QuillV2](./quill-v2/index.mdx) : Mehrzeilige Antwort mit WYSIWYG-Editor : optionaler Typ: `quill-v2` `state` : `<TaskState />` Komponente [TaskState](./task-state/index.mdx) : Zustandsauswahl : optionaler Typ: `task-state`
...
:::warning[Voraussetzung] Damit auch TaskStates, die von einer `<Answer />` Komponente dargestellt werden, in die **Seitenübersicht** aufgenommen werden, braucht es das Plugin `remark-enumerate-components`
