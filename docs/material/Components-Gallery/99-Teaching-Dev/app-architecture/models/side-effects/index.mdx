...
:::tip[Reaktivität] Ein Side Effect stellt das Feld `transformer` zur Verfügung, welches die Transformations-Funktion `(document: TypeDataMapping[Type]) => TypeDataMapping[Type]` enthält. Dank **mobx** ist es möglich, den `transformer` computed zu machen, um automatisch die Änderungen zu verfolgen.
...
const TransformedTaskState = observer(() => {
    ...
    React.useEffect(() => {
        if (doc) {
            doc.registerSideEffect(new RandomState(userStore));
        }
    }, [doc, userStore]);
    return (
        <>
