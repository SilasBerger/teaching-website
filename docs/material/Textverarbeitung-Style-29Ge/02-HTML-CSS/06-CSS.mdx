---
sidebar_label: CSS
page_id: 2303ec99-771f-417f-9a89-9e6356578dc4
---
...
:::insight[Ein Kopf, ein Körper]
Pro HTML-Datei gibt es genau ein `<body>`-Element und genau (oder besser: höchstens) ein `<head>`-Element. Falls Sie plötzlich mehrere `<head>`-Elemente haben, stellen Sie sich am besten einen Menschen vor… 😉
:::
...
:::insight[Ein `<style>`-Element, mehrere Regeln]
Auch wenn Sie mehrere CSS-Regeln definieren möchten (z.B. eine Regel für `h1`, eine Regel für `p`, etc.), brauchen Sie dafür **nicht** mehrere `<style>`-Elemente. Alle Regeln können in einem einzigen `<style>`-Element definiert werden.
:::
...
:::insight[Hex-Code]
Genau die gleiche Farbe könnten wir auch wie folgt definieren:
```css showLineNumbers
h1 {
  color: #FF6347;
}
