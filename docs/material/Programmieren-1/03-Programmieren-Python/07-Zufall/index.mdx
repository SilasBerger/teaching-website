...
:::insight[randint ist Nicht Turtle-spezifisch] Bisher haben wir Python nur in Zusammenhang mit der Turtle verwendet. Man kann mit Python aber natürlich auch ganz andere Dinge machen, die nichts mit zeichnenden Schildkröten zu tun haben! Das werden Sie bald selbst sehen.
...
Zeichnen Sie mit der Turtle ein Quadrat, dessen Seitenlänge jedes Mal zufällige festgelegt wird. Die
Seitenlänge soll immer mindestens 15 Pixel und höchstens 200 Pixel betragen.
...
for i in range(4): forward(seitenlaenge) right(90)
...
for i in range(anzahl_seiten): forward(50) left(360 / anzahl_seiten)
...
bgcolor('black') pencolor('yellow')

anzahl_punkte = randint(30, 100)

for i in range(anzahl_punkte): x = randint(-250, 250) y = randint(-250, 250) goto(x, y) dot(randint(5, 15))
...
:::tip[Repetitive Farben] Es lässt sich dabei (vorerst) nicht vermeiden, dass ab und zu die gleiche Farbe mehrfach vorkommt. :::
...
farben = ['red', 'green', 'blue', 'cyan', 'magenta', 'yellow'] durchmesser = 200

for i in range(5): zufaellige_farbe = choice(farben) pencolor(zufaellige_farbe) dot(durchmesser) durchmesser = durchmesser - 40
...
farben = ['red', 'green', 'blue', 'cyan', 'magenta', 'yellow'] anzahl_seiten = randint(3, 20) seitenlaenge = randint(50, 100)

for i in range(anzahl_seiten): forward(seitenlaenge) pencolor(choice(farben)) left(360 / anzahl_seiten)
