...
:::key[while und if] while : Schleife (Wiederholung) : Körper wird null- bis unendlich-mal wiederholt if : Verzweigung (bedingte Ausführung) : Körper wird entweder null- oder einmal ausgeführt :::
...
:::insight[Vergleichsoperatoren] In dieser Tabelle finden Sie die wichtigsten Vergleichsoperatoren zum Ausdrücken von Bedingungen. | Operator | Bedeutung | |----------|-------------------------| | &lt; | kleiner als | | &lt;= | kleiner oder gleich | | &gt; | grösser als | | &gt;= | grösser oder gleich | | == | gleich | | != | nicht gleich (ungleich) |
...
wurf = randint(1, 6) print('Gewürfelt: ' + str(wurf))

if wurf == 5: print('Du hast gewonnen!') else: print('Du hast verloren...')
...
if n <= 5: pencolor('red') else: pencolor('green')

for i in range(n): forward(100) right(360 / n)
...
for i in range(n): zufallszahl = randint(0, 1) if zufallszahl == 0: pencolor('red') else: pencolor('green') forward(100) right(360 / n)
...
for i in range(n): if n <= 8: forward(100) else: forward(50) right(360 / n)
