...
<Solution id="80daaa81-7c8c-4719-87de-413a8cbfca36">
    Auf Zeile 6 geben wir in den Klammern keine Zahl an, sondern ein `i`. Dieses `i` hat mit dem Start der
    `for`-Schleife auf Zeile 5 zu tun, und wir nennen es den **Schleifenzähler**.
    <br />
    ...
    <br />
    Wenn wir auf der Zeile 6 also `forward(i)` schreiben, dann ist das beim ersten Durchlauf das gleiche wie
    `forward(0)`, beim zweiten Durchlauf so wie `forward(1)`, ..., und beim 100-sten Durchlauf so wie
    `forward(99)`.
</Solution>
::::
...
shape('turtle') pencolor('magenta')

for i in range(100): forward(i) left(30)
...
for i in range(50): right(90) forward(i)
...
shape('turtle') speed(9)

for i in range(28): goto(0, 0) forward(200) right(i)
...
right(90) penup() back(200)

for i in range(3): pendown() for i in range(60): pensize(i) forward(1)
