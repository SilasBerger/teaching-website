...
:::danger[Keine Frage, sondern ein Befehl...] Denken Sie daran: Beim Programmieren ist `zaehler = 0` **keine Frage, sondern ein Befehl**. Wir befehlen dem Computer, den Wert der Variable `zaehler` auf `0` zu setzen.
...
<Solution id="7959dfe9-3168-4eb5-9ef0-0035df0e77aa">
    ...
    <br />
    Wir wissen das, weil der _Datenfluss_ immer **von rechts nach links** geht. `a` hat zuerst den Wert $12$
    und `b` den Wert $7$. Mit `b = a` befehlen wir dem Computer, dass er den Wert der Variable `b` auf den
    aktuellen Wert der Variable `a` setzen soll. Der Wert der Variable `a` ist zu dem Zeitpunkt $12$, also hat
    `b` danach ebenfalls den Wert $12$.
    <br />
    Und ja, die Zuweisung auf Zeile 2 (`b = 7`) ist damit unnötig, weil `b` danach gleich wieder
    **überschrieben** wird 😉.
</Solution>
::::
...
<String
    ...
    solution="str"
    id="d6e9f88e-606e-47a6-bfdb-338ff7473539"
/>
::::

## Programmieraufgaben

:::insight[Mathematische Operatoren] Dies sind die wichtigsten mathematischen Operatoren in Python:
...
:::tip[Drehung] Schlussendlich muss sich die Turtle bei einem beliebigen `n`-Eck immer einmal um insgesamt 360° drehen. :::
...
for i in range(5): forward(seite) left(360 / n)
...
zaehler = 0 while zaehler < n: forward(seite) left(360 / n) zaehler = zaehler + 1
...
durchmesser = umfang / 3.14 radius = durchmesser / 2
...
laenge_aeste = 75 farbe_blaetter = 'green' durchmesser_blaetter = 80

left(90)

# Zeichne den Stamm

pensize(40) pencolor('brown') forward(90)

# Zeichne den ersten Ast (links)

pensize(20) left(45) forward(laenge_aeste) pencolor(farbe_blaetter) dot(durchmesser_blaetter)

# Zeichne den zweiten Ast (Mitte)

penup() back(laenge_aeste) right(45) pendown() pencolor('brown') forward(laenge_aeste) pencolor(farbe_blaetter) dot(durchmesser_blaetter)
