...
Im Ergebnis erhalten Sie allerdings noch kaum Informationen über die entsprechenden Freunde, ausser deren `id`. ![](img/freunde.png)
...
:::aufgabe[Erster Versuch] Nehmen Sie die Abfrage
...
<details>
    ...
    <div>
        ```sql SELECT * FROM legodudes, legodudes_freunde, legodudes WHERE name = 'Botot' and legodude_id =
        legodudes.id; ```
    </div>
</details>
:::
...
:::aufgabe[Abfrage verbessern] Passen Sie Ihre Abfrage so lange an, bis Sie als Ergebnis die folgende Tabelle erhalten: ![--width=60%](img/freunde_mit_namen.png)
