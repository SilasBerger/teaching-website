...
Jedes Element in einer Liste hat einen sogenannten **Index** – also eine _Position_ in der Liste, beginnend mit `0` für das erste Element. Um auf ein bestimmmtes Element in einer Liste zuzugreifen, verwenden wir den Variablennamen der Liste, gefolgt von eckigen Klammern mit dem gewünschten Index:
...
Der Index `5` ist ungültig, weil Listen in Python bei `0` beginnen zu zählen. Wir haben 5 Elemente in der Liste, deren Indizes (Mehrzahl von _Index_) somit von `0` bis `4` gehen. Ein Zugriff auf den Index `5` führt zu einem Fehler, da es kein sechstes Element in der Liste gibt.
</Solution>
:::
...
Weiter können wir mit dem **Slicing**-Operator `:` auch auf mehrere Elemente gleichzeitig zugreifen – also _Teilliste_ ausgeben:
...
4. Passen Sie die erste Zeile so an, dass der _Papageifisch_ und der _Labrador_ die Plätze tauschen (also der _Papageifisch_ an erster Stelle und der _Labrador_ an zweiter Stelle steht). Führen Sie das Programm erneut aus und beobachten Sie, wie sich die Ausgaben ändern.
...
tiere = ['Labrador', 'Papageifisch', 'Capybara', 'Muräne', 'Iguana']
...
Die **Laufvariable** `i` nimmt dabei nacheinander die Werte `0`, `1`, `2`, `3` und `4` an. Tatsächlich generiert die Funktion `range(5)` eine Art Liste[^1] mit diesen Werten. Die obige Schleife können wir also auch so schreiben:
...
Manchmal kann es nützlich sein, während der Iteration auch auf den aktuellen _Index_ zuzugreifen. Das erreichen wir mit der eingebauten Funktion `enumerate()`, die uns sowohl den Index als auch das Element zurückgibt:
...
:::aufgabe[🏆 Challenge: Zyklische Iteration mit mehreren Listen]
...
### Listen verändern
Listen sind **veränderbar** (engl. _mutable_). Das bedeutet, dass wir Elemente in einer Liste hinzufügen, entfernen oder ändern können. Um ein spezifisches Element in der Liste zu ersetzen, verwenden wir dieselbe Syntax, die wir auch zum Zugriff auf Elemente brauchen:
...
Wenn wir ein Element am Ende der Liste hinzufügen möchten, verwenden wir die Methode `append()` (dt.: _anhängen_):
...
```
## Tupel
Tupel verhalten sich sehr ähnlich wie Listen und werden hier deshalb nur kurz vorgestellt. Der Hauptunterschied ist, dass Tupel **unveränderbar** (engl. _immutable_) sind – das bedeutet, dass wir nach der Erstellung eines Tupels keine Elemente hinzufügen, entfernen oder ändern können.
...
_Unveränderbare_ Datentypen haben den Vorteil, dass sie gewissermassen «sicherer» sind, da sie nicht versehentlich verändert werden können. Tupel werden oft verwendet, um Daten zu gruppieren, die zusammengehören und nicht verändert werden sollen.
