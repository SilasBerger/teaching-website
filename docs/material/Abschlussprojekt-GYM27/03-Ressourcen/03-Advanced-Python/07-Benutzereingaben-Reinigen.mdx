...
:::insight[Was ist hier passiert?] Der Wert der Variable `entscheidung` entspricht in diesen Fällen nicht genau dem String `"ja"`. Somit ist die Bedingung auf Zeile `3` `False` und wir führen den `else`-Fall auf Zeile `6` aus. :::
