...
:::aufgabe[Was fällt Ihnen auf?] Was fällt Ihnen in dieser Tabelle auf? Wie könnten Sie diese Erkenntnis unter Umständen nutzen, um doch auf das ursprüngliche Passwort zu schliessen?

Diskutieren Sie zu zweit und vergleichen Sie anschliessend mit der Musterlösung unten.
...
:::aufgabe[Hashes knacken] Alle Benutzer in der obigen Benutzerdatenbank haben ein unsicheres, häufig verwendetes Passwort gewählt: jeder dieser Passwort-Hashes ist in den einschlägigen _Rainbow Tables_ zu finden.
...
Entwickler verwenden zwei wichtige Strategien, um ihre Benutzer im Falle eines Datenlecks vor der Verwendung von Rainbow Tables zu schützen: **Salts** und **Peppers**. Diese beiden Konzepte werden im folgenden Video erklärt: ::youtube[https://www.youtube.com/embed/--tnZMuoK3E?si=bphabzB78ZbBYTAi&amp;start=117]

:::aufgabe[Passwörter salzen] Sie kennen bereits die Passwörter von `alice`, `charlie` und `tammy`. Nun wechseln Sie von der bösen auf die gute Seite.

Legen Sie einen beliebigen _Salt_ fest und erstellen Sie anschliessend eine neue Benutzertabelle mit Einträgen für diese User. Verwenden Sie beim Hashen der Passwörter diesmal aber den gewählten _Salt_. Überprüfen Sie dann nochmal mit [CrackStation](https://crackstation.net/), ob die Benutzer jetzt wirklich gegen die Verwendung von Rainbow Tables geschützt sind.
