---
page_id: 9dec847e-54c0-4d04-a2b4-df9802d4af39
---
...
:::warning[In VSCode ausprobieren]
Komplexere Programme wie Text Adventures lassen sich nicht gut in Classrooms ausführen. Laden Sie das Programm herunter (:mdi[download]), öffnen Sie es mit Thonny und führen Sie es dort aus.
...
Kopieren Sie den folgenden Beispielcode ins VSCode. Passen Sie ihn so an, dass die Spielerin so lange immer wieder nach einer Eingabe gefragt wird, bis sie eine gültige Eingabe macht. Wenn die Eingabe ungültig ist, soll eine entsprechende Meldung ausgegeben werden, und die Spielerin soll erneut nach einer Eingabe gefragt werden.
...
:::insight[Empfohlene Struktur]
Damit Ihr Text Adventure übersichtlich bleibt und Sie Verzweigungen und falsche Benutzereingaben gut handhaben können, empfiehlt es sich, für jede Entscheidungssituation (jede Szene) eine eigene Funktion zu erstellen. So können Sie den Code für jede Situation getrennt schreiben und müssen sich nicht mit zu vielen verschachtelten `if`-Anweisungen herumschlagen.

Wie das geht, lernen Sie Schritt für Schritt im [Tutorial](./05-Tutorial-Text-Adventure/index.mdx).
:::
